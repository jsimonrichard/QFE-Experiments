...
from config import Dataset, Embedder, ClassicalModel, Pooling, gen_args, EnumAction
from train import run_experiment


def objective(trial, pre_args):

    process_name = f"process_out/{pre_args.dataset.value}_{pre_args.embedder.value}"
...
if __name__ == "__main__":
    ...
    study_name = f"{args.dataset.value}_{args.embedder.value}_{'hgp-sl' if args.hgp_sl else 'simple-pooling'}-0.2_test"
